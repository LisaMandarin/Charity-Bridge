...
export function ProductStorageProvider(props) {
  ...
  async function deleteFile(id) {
    ...
    try {
      await productStorage.deleteFile(BUCKET_PRODUCT_ID, id);
  ...
  async function getPreviewURL(id) {
    ...
    try {
      const response = productStorage.getFilePreview(BUCKET_PRODUCT_ID, id);
  ...
  return (
    <ProductStorageContext.Provider
      value={{ fileId, loading, createFile, deleteFile, getPreviewURL }}
    >
      {props.children}
    </ProductStorageContext.Provider>
  );
